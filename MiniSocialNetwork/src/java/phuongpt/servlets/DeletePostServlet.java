...
public class DeletePostServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            //Delete article
            if (dao.deleteArticle(postID)) {
                CommentDAO commentDAO = new CommentDAO();
                EmotionDAO emotionDAO = new EmotionDAO();
                commentDAO.deleteComment(postID);
                emotionDAO.deleteEmotion(postID);
                request.setAttribute("DELETE_SUCCESS", "Delete article successfully!");
            } else {
                request.setAttribute("DELETE_FAIL", "Delete article fail!");
            }
        } catch (SQLException ex) {
            log("DeletePostServlet _ SQL " + ex.getMessage());
        } catch (NamingException ex) {
            log("DeletePostServlet _ Naming " + ex.getMessage());    
        } finally {
